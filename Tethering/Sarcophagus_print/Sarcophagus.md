...
`Sarcophagus_platform_15.FCStd`: We printed this design in-house with different materials (including ABS and resins) and with different material densities. We then immobilized flies by placing their vials in a container of ice until they stopped moving and positioned individual flies in the different printouts. The flies in the low-density prints were the first ones able to climb out of their groove in less than two minutes. Independent of the material, the flies in the high-density prints stayed in place considerably longer, up to 10 minutes. This gives enough time to tether a fly and we were able to use the 3D printed platforms for the teaching course. We also outsourced metal printing from aluminum and stainless steel materials. While still below $10 per piece, they are a little more expensive yet transmit temperature very well. This results in flies being cooled to the temperature of the peltier element and allows us to keep the flies indefinitely in a chill coma.

`Sarcophagus_platform_2.FCStd`: A reduced version of the same platform with only 2 sarcophagus slots. The larger groove is meant to work with an average female _Drosophila_ while the smaller indentation usually fits the males. The print-ready STL is `production/Sarcophagus_platform_2.stl`.

`Sarcophagus_platform_3_BumbleBee.FCStd`: A platform designed specifically for bumblebees, with 3 sarcophagus slots sized for a bumblebee thorax. The design follows the same principle as the fly platforms — printed at high density and placed on the Peltier element with thermal adhesive tape. Two versions of the production STL are available: `production/Sarcophagus_platform_3_BumbleBee_v0.1.stl` and the refined `production/Sarcophagus_platform_3_BumbleBee_v0.2.stl`.
